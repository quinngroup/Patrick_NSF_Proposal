...
I want to use a variety of simplistic models such as linear regression and decision trees to determine the features that contribute to whether a tweet is associated with a certain personality or mentions a certain disease negatively or positively. This simple models are useful because they can more easily be interpreted. Next I want to do a variety of hypothesis tests to determine if certain diseases or personality traits correlate significantly with each other. This correlation will have to take into account whether or not the disease or personality scores are continuous or discrete, though this will be decided on at the time.

Taking advantage of metadata available in tweets, We can determine if there are any correlations or interesting summary statistics that correlate with disease or personality variables. Given two personality types or two sets of positive or negative disease-related tweets, we can measure the most frequent domain name prefixes in the hyper links. We can also measure if there are non-random temporal or spacial (using geolocation metadata) distribution of these tweets relative to some background distribution (hence the need for a set of background tweets).

- Also want to incorporate deep learning (perhaps generative / or semi supervised or whatever) just to have an excuse to do machine learning

- Get more coherent specific with goals and methods. What are the expected benefits that would come from this research??

- Think of graphics / diagrams / figures to add for this section
